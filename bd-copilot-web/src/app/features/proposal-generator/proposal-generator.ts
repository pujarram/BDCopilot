...
export class ProposalGenerator implements OnInit, OnDestroy {
    ...
    if (!saved) {
      ...
      this.api.saveDocumentHistory(HISTORY_TYPE, {
        ...
        metadata: {
          ...
          includeDeck: String(this.includeDeck()),
          includeArchitectureDiagram: String(this.includeArchitectureDiagram())
        }
      }).subscribe({
        next: row => {
  ...
  protected exportHistory(item: GeneratedDocumentHistoryListItem): void {
    this.exporting.set(true);
    this.api.exportDocumentHistory(HISTORY_TYPE, item.id, this.teams.user().objectId, 'docx').subscribe({
      next: result => {
    ...
    this.api.saveDocumentHistoryToChannel(HISTORY_TYPE, id, this.teams.user().objectId).subscribe({
      next: row => {
        ...
        if (row.channelUploadStatus === 'Uploaded') {
          this.toast.show('Uploaded to BD channel Proposals folder.');
  ...
  private persistToDatabase(doc: GeneratedDocument, showToast = false): void {
    ...
    this.api.saveDocumentHistory(HISTORY_TYPE, {
      ...
      next: row => {
        ...
        this.saving.set(false);
        this.refreshHistory();
        if (showToast) this.toast.show('Saved to proposal history.');
      },
      error: err => {
        console.error(err);
        this.saving.set(false);
  ...
  private async runLiveStream(signal: AbortSignal): Promise<void> {
    try {
      const stream = this.api.generateProposalStream(
        {
          solution: this.solution(),
